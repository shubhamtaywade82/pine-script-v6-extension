...
## tradingview-mcp workspace

Bundled docs: `vendor/pinescript-agents/docs/` (from [TradersPost/pinescript-agents](https://github.com/TradersPost/pinescript-agents)). Skip `/projects/blank.pine` rotation; use workspace `*.pine` files. For live TradingView Desktop control, follow repo `CLAUDE.md` MCP Pine tools.
...
1. Standard flow: `vendor/pinescript-agents/docs/project-scoping-flow.md` and `vendor/pinescript-agents/docs/scoping-questions.md`
2. Unknown patterns: `vendor/pinescript-agents/docs/comprehensive-scoping-flow.md`
3. Edge cases: `vendor/pinescript-agents/docs/edge-case-handler.md`
...
2. Check `vendor/pinescript-agents/docs/edge-case-handler.md` for workarounds
...
- **Create or name the workspace `*.pine` file as soon as the project is defined**
...
### When Starting Any New Project (this workspace)

1. Create or choose a workspace `*.pine` file with a descriptive name (e.g. `rsi-divergence-indicator.pine`, `ma-crossover-strategy.pine`); use lowercase with hyphens when creating new files unless the user prefers otherwise.

2. Update the file header with project details.

3. Begin the development workflow.

### Upstream blank.pine rotation

The original [pinescript-agents](https://github.com/TradersPost/pinescript-agents) repo uses `/projects/blank.pine` rotation. **That layout is not present here**—do not require blank.pine unless the user adds it.
...
```
1. CREATE/OPEN: [indicator-name].pine in the workspace
...
```
1. CREATE/OPEN: [strategy-name].pine in the workspace
