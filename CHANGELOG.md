...
- **Hover / signature help:** optional **`documentation`** (Markdown) per symbol in [`src/references/signatureOverlay.json`](src/references/signatureOverlay.json) is shown in the editor together with the curated **`signature`** and the TradingView link (starts with **`alertcondition`** — const-string / **CE10123** notes).
