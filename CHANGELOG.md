...
- **Hover / signature help:** TradingView-style layout — **`built-in function`** label, optional **`overloadHint`** (e.g. `+4 overloads`), prose then a **`Syntax`** fenced block, then the TradingView link. Curated **`documentation`** + **`signature`** for **`math.round`**, **`str.tostring`**, **`color.new`**, **`plotshape`**, and **`alertcondition`** (grow the overlay for more symbols).
