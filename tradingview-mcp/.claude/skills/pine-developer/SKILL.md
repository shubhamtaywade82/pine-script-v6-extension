...
Bundled docs from [TradersPost/pinescript-agents](https://github.com/TradersPost/pinescript-agents) live under `vendor/pinescript-agents/docs/` (paths below use that prefix). There is no `/projects/` tree in this repo: save scripts as `*.pine` where the user wants (often repo root). With TradingView Desktop + MCP connected, use `pine_set_source`, `pine_smart_compile`, `pine_get_errors`, `pine_get_console`, and `pine_save` per repo `CLAUDE.md`.
...
- `vendor/pinescript-agents/docs/pinescript-v6/quick-reference/syntax-basics.md` - Core syntax and structure
- `vendor/pinescript-agents/docs/pinescript-v6/reference-tables/function-index.md` - Complete function reference
- `vendor/pinescript-agents/docs/pinescript-v6/core-concepts/execution-model.md` - Understanding Pine Script execution
- `vendor/pinescript-agents/docs/pinescript-v6/core-concepts/repainting.md` - Avoiding repainting issues
- `vendor/pinescript-agents/docs/pinescript-v6/quick-reference/limitations.md` - Platform limits and workarounds

Load these docs as needed based on the task at hand.

## Project File Management

- Create or edit a `*.pine` file in this workspace (repo root or a path the user specifies); skip upstream `blank.pine` / `/projects/` rotation unless the user uses that layout elsewhere
- Never create extra files unless needed for multi-file projects
